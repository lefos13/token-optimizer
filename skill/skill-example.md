...
Command temp-stream and final audit failures never override exit/signal truth. Active `.active.tmp` output is not lifecycle-managed while fresh; open/write/end, fsync, close, and abort cleanup report `removed` or `failed`. After fsync and close, final-rename failure must first atomically mark evidence `.retained.audit.tmp`; only then is it registered and lifecycle-managed. If marking and deletion both fail, inspect the contained `orphanPath`; it is not registered and becomes stale-active recovery input after one hour. Treat registry, log-lifecycle, and analytics persistence entries in `warnings` as operational advisories.
