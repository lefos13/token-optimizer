...
Per-command output streams are finalized before suite assembly. Open, write, disk-full, permission, and finalization failures therefore preserve authoritative exit/signal data while reporting audit failure separately. In-progress `.active.tmp` files are excluded from status, retention, quota pruning, and purge. Fsync and close failures remove them; after successful fsync and close, a final-rename failure atomically marks the file `.retained.audit.tmp` before it can be reported or registered. If that marking fails, the active file is deleted. Retained evidence participates in lifecycle operations exactly like `.log` runs. Registry, lifecycle, and local analytics persistence failures remain non-fatal and appear in `warnings`.
...
Command temp-stream open/write/end failures are awaited and reported as audit failures; incomplete `.active.tmp` evidence is removed and excluded from lifecycle operations while a run is open. After fsync and close, rename failure atomically transitions the file to `.retained.audit.tmp`; only that state is registered and lifecycle-managed. A failed transition deletes the active file and exposes no evidence path. Registry, lifecycle, and analytics persistence remain best-effort and add warnings to the same response.
