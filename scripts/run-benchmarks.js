...
async function main() {
  ...
  const report = { schemaVersion: 2, release: '2.0.0-rc.3', ...before, timestamp: new Date().toISOString(), platform: `${process.platform}-${process.arch}`,
  ...
  const output = path.join(root, 'benchmarks/results/v2.0.0-rc.3.json'); fs.mkdirSync(path.dirname(output), { recursive: true }); fs.writeFileSync(output, `${JSON.stringify(report, null, 2)}\n`); console.log(JSON.stringify({ output: safe(output), workloads: workloads.length }));
}
