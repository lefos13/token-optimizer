...
try {
  ...
  /* Launcher: self-locates via $(dirname "${BASH_SOURCE[0]}") rather than an
     Antigravity-provided "plugin root" env var (none is documented), so the
     plugin works wherever Antigravity stages it. Installs the runtime dep into
     a persistent .data/ directory beside the server on first run (or when
     package.json changes), then execs the server. Install chatter goes to
