/* Shared cross-platform Node launcher emitted as server/start.js by every
   ...
   Behavior mirrors start.sh: install the single runtime dep into a persistent
   data dir when the manifest changes, keep stdout reserved for JSON-RPC, then
   run the server with NODE_PATH pointing at the installed deps. */
