...
| --- | --- | --- |
| Compromised MCP client, prompt injection, malicious repository, arbitrary command profiles | Deny-first parsing in `src/command-policy.ts`; `safe` allowlists, `standard` adds detected commands, and `unrestricted` still denies escape, secrets, network, destructive syntax, nesting, chaining, and substitution. `src/runner.ts` evaluates before spawn and `src/process-tree.ts` kills descendants. | `test/security/command-boundary.test.ts`, `test/client/command-policy.test.ts`, `test/client/process-tree.test.ts` |
...
| Package supply chain | Lockfile, generated-asset checks, aligned release versions, pack inspection, and audit are release gates; launchers install only the declared runtime dependency. | `test/scripts/package-hygiene.test.ts`, `test/scripts/plugin-generators.test.ts`, `test/scripts/release-versions.test.ts` |
