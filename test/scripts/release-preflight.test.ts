...
function adapters(overrides: Record<string, unknown> = {}) {
  ...
  return {
    env: { RELEASE_TAG: 'v2.0.13', RELEASE_ARTIFACT_DIR: `release-artifacts/test-${process.pid}-${Math.random()}`, PREFLIGHT_ALLOW_DIRTY: '1' },
    argv: [],
...
test('runPreflight returns stable success JSON and derives the dist tag from the release version', () => {
  assert.deepEqual(runPreflight(root, adapters()), { ok: true, code: 'RELEASE_PREFLIGHT_PASSED', version: '2.0.13', distTag: 'latest', warnings: [], artifacts: ['root.cdx.json', 'installer.cdx.json'] });
});
...
test('runPreflight reports dirty, tag, staged-secret, and installer-pack failures', () => {
  const scenarios: Array<[string, Record<string, unknown>]> = [
    ['DIRTY_TREE', { env: { RELEASE_TAG: 'v2.0.13' }, run: (command: string, args: string[]) => ({ status: 0, stdout: command === 'git' && args[0] === 'status' ? ' M tracked.ts\n' : '', stderr: '' }) }],
    ['TAG_REQUIRED', { env: {}, argv: [] }],
    ['TAG_VERSION_MISMATCH', { env: { RELEASE_TAG: 'v2.0.0-beta.10' } }],
    ['TAG_POLICY_REJECTED', { env: { RELEASE_TAG: '2.0.1' } }],
...
test('every git command failure is authoritative and machine-coded', () => {
  for (const operation of ['status', 'diff', 'ls-files']) {
    const base = adapters();
    const env = operation === 'status' ? { RELEASE_TAG: 'v2.0.13' } : base.env;
...
test('CLI emits one machine-readable stderr object and exits exactly one on policy failure', () => {
  ...
  assert.deepEqual(JSON.parse(result.stderr), { ok: false, code: 'TAG_REQUIRED', details: { version: '2.0.13' } });
});
