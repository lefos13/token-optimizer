...
test('marketplace location contract covers cache, alternate, and skills layouts', () => {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'marketplace-layouts-')); const paths = [path.join(home, '.claude', 'plugins', 'cache', 'token-optimizer-marketplace', 'token-optimizer', '2.0.0-beta.6'), path.join(home, '.claude', 'skills', 'token-optimizer'), path.join(home, '.codex', 'plugins', 'cache', 'Softaware-marketplace', 'token-optimizer', '2.0.0-beta.6'), path.join(home, '.codex', 'plugins', 'token-optimizer', '2.0.0-beta.6')]; for (const entry of paths) { fs.mkdirSync(entry, { recursive: true }); fs.writeFileSync(path.join(entry, 'marker'), 'x'); } const adapter = createFilesystemMarketplaceAdapter(home); assert.equal(adapter.list('claude').length, 2); assert.equal(adapter.list('codex').length, 2);
});
