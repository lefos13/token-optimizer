...
test('multi-version marketplace extras coalesce into one normalize cycle', () => {
  ...
  const normalize = plan.operations.filter((item: any) => item.command === 'normalize-marketplace-registration'); assert.equal(normalize.length, 1); assert.equal(normalize[0].identities.length, 2); assert.deepEqual(normalize[0].canonicalIdentity, canonical); assert.deepEqual(normalize[0].recipe.restore, ['plugin', 'install', 'token@2.0.0-beta.6']);
});
