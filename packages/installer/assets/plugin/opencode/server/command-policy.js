 ...
 */
function matchesStandardProfileCommand(command) {
    const args = tokens(command);
    ...
    if (!/^git\s+diff(?:\s|$)/.test(command.trim()))
        return false;
    if (args[0] !== 'git' || args[1] !== 'diff')
        return false;
    return !args.slice(2).some((arg) => arg === '--ext-diff'
        || arg === '--textconv'
        || arg === '--output'
...
async function evaluateCommand(input) {
    ...
    if (input.profile === 'standard' && matchesStandardProfileCommand(command))
        return { allowed: true, profile: input.profile, reasonCode: 'STANDARD_PROFILE' };
