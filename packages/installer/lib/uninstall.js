...
function planRepair(report = {}, manifest, options = {}) {
  ...
  for (const finding of actionable) {
    ...
    if (['rewrite-registration', 'deduplicate-registration', 'install-client'].includes(finding.operation)) { const owned = (manifest.registrations || []).filter((item) => !finding.client || item.client === finding.client); const canonicalOwned = owned.find((item) => item.template); if (canonicalOwned) { const identities = finding.registrations || []; const canonicalPath = finding.canonical?.path || canonicalOwned.canonicalPath; const extras = (finding.paths || (finding.path ? [finding.path] : [])).filter((item) => item !== canonicalPath).filter((item) => { try { return fs.lstatSync(item).isFile(); } catch (_) { return false; } }); for (const external of owned.filter((item) => item.kind === 'marketplace' && identities.some((identity) => identity.type === 'marketplace' && identity.path !== finding.canonical?.path))) operations.push(clientCommandOperation(external.client, 'remove-registration', { paths: [], recipe: { remove: external.remove, restore: external.restore } })); operations.push(clientCommandOperation(canonicalOwned.client, 'upsert-registration', { paths: extras, canonicalPath: canonicalOwned.canonicalPath, template: canonicalOwned.template })); } }
