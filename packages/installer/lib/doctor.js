...
async function inspectInstallation(options = {}) {
  ...
  for (const client of CLIENTS) { const duplicates = registrations.registrations.filter((item) => item.client === client); if (duplicates.length > 1) { const identities = duplicates.map((item) => ({ client, name: item.name, type: item.marketplace ? "marketplace" : "direct", path: item.configPath, installedPath: item.installedPath, version: item.version || null, stale: item.stale === true })); const canonical = identities.find((item) => item.type === "direct" && !item.stale) || identities.find((item) => item.version === expectedVersion && !item.stale) || identities.find((item) => !item.stale) || identities[0]; add(findings, "DUPLICATE_REGISTRATION", "warning", `${client} has multiple Token Optimizer MCP registrations.`, "Keep the canonical managed registration and remove only extras.", { client, paths: identities.map((item) => item.path), registrations: identities, canonical, operation: "deduplicate-registration" }); } }
