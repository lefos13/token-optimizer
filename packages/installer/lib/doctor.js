...
async function inspectInstallation(options = {}) {
  ...
  for (const client of CLIENTS) { const duplicates = registrations.registrations.filter((item) => item.client === client); if (duplicates.length > 1) add(findings, "DUPLICATE_REGISTRATION", "warning", `${client} has multiple Token Optimizer MCP registrations.`, "Keep one managed registration and remove stale duplicates.", { client, paths: duplicates.map((item) => item.configPath), operation: "deduplicate-registration" }); }
