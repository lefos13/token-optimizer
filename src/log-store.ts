...
export async function createRunLog(workspacePath: string, options: { storageMode?: 'raw-local' | 'redacted-local'; runId?: string } = {}): Promise<RunLog> {
  const dir = canonicalDir(workspacePath); await fs.promises.mkdir(dir, { recursive: true });
  const id = options.runId || `${new Date().toISOString().replace(/[:.]/g, '-')}-${process.pid}`;
  const absolutePath = safePath(workspacePath, path.join(dir, `${id}.log`));
  const stream = fs.createWriteStream(absolutePath, { flags: 'w', mode: 0o600 });
  const mode = options.storageMode || DEFAULT_LOG_POLICY.storageMode;
  const write = (chunk: string | Buffer) => new Promise<void>((resolve, reject) => {
    const value = mode === 'redacted-local' ? redactText(Buffer.isBuffer(chunk) ? chunk.toString() : chunk).text : chunk;
    if (!stream.write(value)) stream.once('drain', resolve); else resolve();
    stream.once('error', reject);
  });
  const close = () => new Promise<void>((resolve, reject) => { stream.once('error', reject); stream.end(resolve); });
...
async function entries(workspacePath: string): Promise<Entry[]> {
  const dir = canonicalDir(workspacePath); let names: string[]; try { names = await fs.promises.readdir(dir); } catch { return []; }
  const out: Entry[] = []; for (const name of names) { if (!name.endsWith('.log')) continue; const file = safePath(workspacePath, path.join(dir, name)); const st = await fs.promises.stat(file); out.push({ file, bytes: st.size, mtimeMs: st.mtimeMs }); }
...
export async function purgeLogs(workspacePath: string, options: { includeBaseline?: boolean; includeAnalytics?: boolean } = {}): Promise<LogLifecycleResult> {
  const all = await entries(workspacePath); const removed: RemovedLog[] = []; for (const e of all) { await fs.promises.unlink(e.file); removed.push({ path: path.relative(workspacePath, e.file), bytes: e.bytes, reason: 'purged' }); }
